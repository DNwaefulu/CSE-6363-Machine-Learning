# -*- coding: utf-8 -*-
"""
Created on Wed Jun 28 21:51:40 2023
...
for model in models:
    ...
    sns.lineplot(data=[l.value_losses, l.train_losses]).set(xlabel='Step no.',ylabel='MSE',xlim=(0,None))
    plt.title(model)
    plt.legend(labels=["Validation MSE","Train MSE"])
    plt.savefig('{}_result.jpg'.format(model))
    plt.show()
    
    result = l.saveLoad('trainingData.csv')
